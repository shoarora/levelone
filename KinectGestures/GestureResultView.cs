...
namespace Microsoft.Samples.Kinect.DiscreteGestureBasics
{
    ...
    {
        /// <summary> names of gesture that can be detected </summary>
        private readonly string squatProgressGestureName = "SquatProgress3Progress";
        private readonly string jumpProgressGestureName = "JumpingJack";


        private readonly ImageSource squattingImage = new BitmapImage(new Uri(@"Images\squat3.png", UriKind.Relative));
        private readonly ImageSource midSquatImage = new BitmapImage(new Uri(@"Images\squat2.png", UriKind.Relative));
        private readonly ImageSource notSquattingImage = new BitmapImage(new Uri(@"Images\squat1.png", UriKind.Relative));
        private readonly ImageSource jumpingImage = new BitmapImage(new Uri(@"Images\jump3.png", UriKind.Relative));
        private readonly ImageSource midJumpImage = new BitmapImage(new Uri(@"Images\jump2.png", UriKind.Relative));
        private readonly ImageSource notJumpingImage = new BitmapImage(new Uri(@"Images\jump1.png", UriKind.Relative));
        ...
        /// <summary> Current progress value reported by the discrete gesture </summary>
        ...
        /// <param name="progress">Progress value for detection of the 'Seated' gesture</param>
        public GestureResultView(int bodyIndex, bool isTracked, bool detected, float progress)
        {
        ...
        /// <summary> 
        /// Gets a float value which indicates the detector's progress that the gesture is occurring for the associated body 
        /// </summary>
        ...
        /// <param name="gestureProgress">Progress value for detection of the discrete gesture</param>
        /// public void UpdateGestureResult(bool isBodyTrackingIdValid, bool isGestureDetected, float gestureProgress)
        public void UpdateGestureResult(bool isBodyTrackingIdValid, bool isGestureDetected, string gestureName, float gestureProgress)
        {
            ...
            else
            {
                ...
                if (this.Detected)
                {
                    this.Progress = gestureProgress;
                    if (gestureName == squatProgressGestureName)
                    {
                        if (this.Progress >= .25 && this.Progress <= .75)
                        {
                            this.ImageSource = this.midSquatImage;
                        }
                        else if (this.Progress > .75)
                        {
                            this.ImageSource = this.squattingImage;
                        }
                        else
                        {
                            this.ImageSource = this.notSquattingImage;
                        }
                    }
                    else if (gestureName == jumpProgressGestureName)
                    {
                        if (this.Progress >= .25 && this.Progress <= .75)
                        {
                            this.ImageSource = this.midJumpImage;
                        }
                        else if (this.Progress > .75)
                        {
                            this.ImageSource = this.jumpingImage;
                        }
                        else
                        {
                            this.ImageSource = this.notJumpingImage;
                        }
                    }
                    else {
                        this.ImageSource = this.notJumpingImage;
